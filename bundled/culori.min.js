var culori = (() => {
	...
	var ee = (e, t) => {
		...
		Yt = (e, t) => {
			...
			let r = T(e.h),
				o = T(t.h);
			return Math.abs(o - r) > 180
				? r - (o - 360 * Math.sign(o - r))
				: o - r;
		},
		te = (e, t) => {
				...
				let u = r(n),
					...
					X =
						s >= (164 / 180) * Math.PI && s <= (345 / 180) * Math.PI
							? 0.56 +
							  Math.abs(
									0.2 * Math.cos(s + (168 / 180) * Math.PI)
							  )
							: 0.36 +
							  Math.abs(
									0.4 * Math.cos(s + (35 / 180) * Math.PI)
							  ),
	...
	function We(e) {
		...
		return (
			f && ((t.s = f), (t.h = T((Math.atan2(e.b, e.a) * 180) / Math.PI))),
			t
		);
	}
	function Ue(e) {
	...
	var xf =
			...
			t =>
				e <= 0
					...
					: e >= 1
					? 0
					: Math.pow(t, Math.log(0.5) / Math.log(e)),
		lr = xf;
	var ir = e => typeof e == 'function',
		...
		a0 = (e, t = 'rgb', r, o) => {
			...
			if (o) {
				let h = f.map((d, c) =>
					n.channels.reduce((g, M) => ((g[M] = m[M][c]), g), {
						mode: t
					})
				);
				m = n.channels.reduce(
	...
	var zf = (e, t = Oe(), r = o => o) => {
		...
		M0 = zf;
	var kf = (e, t, r) =>
			e === void 0 || t === void 0 ? void 0 : e + r * (t - e),
		v0 = kf;
