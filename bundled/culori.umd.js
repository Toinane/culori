(function (root, factory) {
	...
	var culori = (() => {
		...
		var parseRgb = color => {
			...
			let match;
			if (
				(match = color.match(rgb_num_old) || color.match(rgb_num_new))
			) {
				if (match[1] !== void 0) {
					res.r = match[1] / 255;
				}
		...
		var convertRgbToLab65 = rgb5 => {
			let res = convertXyz65ToLab65_default(
				convertRgbToXyz65_default(rgb5)
			);
		...
		var convertRgbToLab = rgb5 => {
			let res = convertXyz50ToLab_default(
				convertRgbToXyz50_default(rgb5)
			);
			if (rgb5.r === rgb5.b && rgb5.b === rgb5.g) {
				res.a = res.b = 0;
			}
		...
		var deficiency = (lut, t) => {
			...
			if (w > 0 && i < lut.length - 1) {
				let arr_2 = lut[i + 1];
				arr = arr.map((v, idx) =>
					lerp_default(arr[idx], arr_2[idx], w)
				);
			}
			return color => {
